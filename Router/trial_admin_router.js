...
const flightdetails = require("../Controller/trial_admin_controller");
...
router.get("/newflight", async (req, res) => {
  ...
  {    
      ...
      var n = d.length;
      console.log(n)
      res.render('trialscheduleflights', { layout: false,  flight:d, detail: logindetails, flight_id: " " });
...
router.post("/all_schedules", async (req, res) => {
    ...
    if (name.deleteschedule != undefined) {
        ...
        res.render('trialscheduleflights', { layout: false, flight:a,flight_id:req.body});
 }
 else
...
router.post("/edit_schedule", async (req, res) => {
 ...
 var a = await flightdetails.flightlist(req, res);
 var d = await flightdetails.get_flight(req, res);
 res.render('trialscheduleflights', { layout: false, flight:a,flight_id:req.body.flight_id })
})
...
router.post('/update_form',async(req,res)=>{
    ...
    //     await all_questions.addtoevents(req, res);
        res.render('trialscheduleflights', { layout: false,Question:Edit, flight:d ,flightdetails:submit ,flight_id:submit.flight_id})
        console.log("hfvtb")
